...
from .tools_files import _ensure_safe, _resolve_file
...
def run_shell_command(args: Dict[str, Any]) -> Dict[str, Any]:
    """Execute an arbitrary PowerShell or cmd command on Windows and return stdout/stderr.

    IMPORTANT: Only run safe, user-requested commands. Do NOT run destructive
    commands (rm -rf, format, del /S, etc.) without explicit user confirmation.
    Prefer PowerShell for Windows system tasks (execution policy, registry, etc.)
    """
    command = args.get("command") or args.get("cmd")
    if not command:
        raise ToolError("Parameter 'command' is required.")

    shell_type = str(args.get("shell", "powershell")).strip().lower()
    timeout = int(args.get("timeout", 30))
    cwd = args.get("cwd") or os.getcwd()

    # Safety: block outright destructive one-liners
    BLOCKED = [
        "format ", "del /s", "rmdir /s", "rm -rf", "remove-item -recurse",
        "rd /s /q", "> /dev/", "dd if=",
    ]
    ...
    for b in BLOCKED:
        if b in cmd_lower:
            raise ToolError(
                f"Command blocked for safety (matches '{b}'). "
                "Please ask the user to run this manually."
            )

    if shell_type == "cmd":
        full_cmd = ["cmd.exe", "/C", str(command)]
    else:
        # Default: PowerShell — bypass execution policy for this invocation only
        full_cmd = [
            "powershell.exe",
            "-NonInteractive",
            "-NoProfile",
